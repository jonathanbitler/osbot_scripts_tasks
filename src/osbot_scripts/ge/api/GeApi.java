...
public final class GeApi {
	...
	public final Map<String, ExchangeItem> getExchangeItems(final String... itemNames) {
		...
		for (final String itemName : itemNames) {
			getItemID(ITEMS_JSON, itemName)
					.ifPresent(id -> exchangeItems.put(itemName, new ExchangeItem(itemName, id)));
		}
	...
	private Optional<String> getItemFromJson(final String json, final String itemName) {
		Matcher matcher = Pattern.compile("(\\{[﻿^}]*\"name\"\\s*:\\s*\"" + Pattern.quote(itemName) + "\"[^}]*})")
				.matcher(json);
